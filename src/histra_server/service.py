...
def register_runner(session: Session, payload: Any) -> Runner:
    ...
    if runner is None:
        ...
        runner.version = payload.version
...
def expire_leases(session: Session, cache: PackageCache, now: datetime | None = None) -> int:
    ...
    expired = session.scalars(
        select(Attempt).where(
            ...
            Attempt.lease_expires_at.is_not(None),
...
def claim_job(
    ...
    *,
    ...
    runner = session.get(Runner, runner_id)
    if runner is None:
        raise NotFoundError("runner is not registered")
    expire_leases(session, cache)

    statement = (
        select(Job)
        .where(Job.status == "queued", Job.attempts_count < Job.max_attempts)
    ...
    try:
        package_bytes, manifest = build_attempt_package(
            job.document,
        ...
        attempt.failure = {"error_type": type(exc).__name__, "message": str(exc)}
        attempt.updated_at = now
    ...
    now = utcnow()
    attempt.status = "leased"
    attempt.hrx_sha256 = manifest["hrx"]["sha256"]
    attempt.package_manifest = manifest
...
def get_package(
    ...
    *,
    ...
    regenerated, manifest = build_attempt_package(
        job.document,
        attempt_id=attempt.id,
        created_at=attempt.created_at,
        registry=registry,
    )
    if attempt.package_manifest != manifest or attempt.hrx_sha256 != manifest["hrx"]["sha256"]:
        raise ConflictError("regenerated package is not identical to the leased package")
    cache.put(attempt.id, regenerated)
    return regenerated
...
def heartbeat(
    ...
    *,
    ...
    if attempt.runner_id != runner_id:
        raise IdentityError("attempt belongs to another runner")
    if attempt.status != "leased":
        raise InvalidStateError(f"cannot heartbeat an attempt in state {attempt.status}")
    now = utcnow()
    attempt.lease_expires_at = now + timedelta(seconds=lease_seconds)
    attempt.updated_at = now
    runner = session.get(Runner, runner_id)
...
def complete_attempt(
    ...
    *,
    ...
    if attempt is None or attempt.job_id != job_id:
        raise NotFoundError("attempt does not exist")
    if attempt.runner_id != payload.runner_id:
        raise IdentityError("attempt belongs to another runner")
    if payload.job_id != job_id or payload.attempt_id != attempt_id:
        raise IdentityError("result identity does not match URL")
    if payload.job_sha256 != attempt.job_sha256 or payload.hrx_sha256 != attempt.hrx_sha256:
        raise IdentityError("result provenance does not match the leased package")

    envelope = payload.model_dump(mode="json")
    if attempt.status == "completed":
        if attempt.result != envelope:
            raise ConflictError("attempt is already completed with different results")
        return attempt
    if attempt.status != "leased":
        raise InvalidStateError(f"cannot complete an attempt in state {attempt.status}")

    now = utcnow()
    attempt.status = "completed"
    attempt.result = envelope
    attempt.logs = payload.logs
    attempt.updated_at = now
    attempt.completed_at = now
    job = session.get(Job, job_id)
    assert job is not None
...
def fail_attempt(
    ...
    *,
    ...
    if attempt.runner_id != payload.runner_id:
        raise IdentityError("attempt belongs to another runner")
    if attempt.status != "leased":
        raise InvalidStateError(f"cannot fail an attempt in state {attempt.status}")
    now = utcnow()
    attempt.status = "failed"
    attempt.failure = {
...
def cancel_job(session: Session, job_id: str, cache: PackageCache) -> Job:
    ...
    for attempt in job.attempts:
        if attempt.status in {"building", "leased"}:
